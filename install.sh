...
# Usage: curl -fsSL https://gitee.com/<owner>/revideo-server/raw/main/install.sh | bash
set -euo pipefail
...
# Release URLs (update owner/repo after creating the Gitee mirror)
readonly GITEE_RELEASE_BASE="https://gitee.com"
readonly GITHUB_RELEASE_BASE="https://github.com"
# These will be set to the actual repo paths
GITEE_REPO="${GITEE_REPO:-agflow/revideo}"
GITHUB_REPO="${GITHUB_REPO:-Match-Yang/revideo-server}"
...
detect_os() {
  case "$(uname -s)" in
    ...
    CYGWIN*|MINGW*|MSYS*)
      ...
      error "  iex (irm https://gitee.com/${GITEE_REPO}/raw/main/install.ps1)"
      exit 1
      ;;
...
get_download_url() {
  ...
  if [[ -n "$GITEE_REPO" ]]; then
    local gitee_url="${GITEE_RELEASE_BASE}/${GITEE_REPO}/releases/download/${version}/${filename}"
    if curl -fsSL --head "$gitee_url" &>/dev/null; then
      echo "$gitee_url"
      return
    fi
  fi
  ...
  # Fallback to GitHub
  local github_url="${GITHUB_RELEASE_BASE}/${GITHUB_REPO}/releases/download/${version}/${filename}"
  echo "$github_url"
}
...
print_banner() {
  printf "\n"
  printf "${CYAN}${BOLD}  ____      _   _            _                ____            _             ${NC}\n"
  printf "${CYAN}${BOLD} |  _ \\ ___| |_| |_ ___ _ __| |__   ___ _ __ |  _ \\ _____  _(_) ___  _ __  ${NC}\n"
  printf "${CYAN}${BOLD} | |_) / _ \\ __| __/ _ \\ '__| '_ \\ / _ \\ '_ \\| |_) / _ \\ \\/ / |/ _ \\| '__| ${NC}\n"
  printf "${CYAN}${BOLD} |  _ <  __/ |_| ||  __/ |  | |_) |  __/ | | |  _ <  __/>  <| | (_) | |    ${NC}\n"
  printf "${CYAN}${BOLD} |_| \\_\\___|\\__|\\__\\___|_|  |_.__/ \\___|_| |_|_| \\_\\___/_/\\_\\_|\\___/|_|    ${NC}\n"
  printf "\n"
