...
# Usage: iex (irm https://gitee.com/<owner>/revideo-server/raw/main/install.ps1)
[CmdletBinding()]
param(
    [switch]$Uninstall,
...
$MinNodeMajor = 22
$GiteeRepo = if ($env:GITEE_REPO) { $env:GITEE_REPO } else { "agflow/revideo" }
...
function Get-LatestVersion {
    ...
    if ($GiteeRepo) {
        try {
            $resp = Invoke-RestMethod "https://gitee.com/api/v5/repos/$GiteeRepo/releases/latest" -ErrorAction Stop
    ...
    if (-not $version) {
        try {
            $resp = Invoke-RestMethod "https://api.github.com/repos/$GitHubRepo/releases/latest" -ErrorAction Stop
            $version = $resp.tag_name
        } catch {}
    }
...
function Download-AndExtract {
    ...
    # Try Gitee first
    if ($GiteeRepo) {
        $giteeUrl = "https://gitee.com/$GiteeRepo/releases/download/$ver/$filename"
        try {
            Invoke-WebRequest $giteeUrl -Method Head -ErrorAction Stop | Out-Null
            $url = $giteeUrl
        } catch {}
    }
